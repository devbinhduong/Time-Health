...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        $(document).on(
            'click',
            '#form-action-addToCart2.halothemes',
            (event) => {
                $form2.submit();
            }
        );
    ...
    setProductVariant() {
        ...
        $.each(
            $('[data-product-option-change] [data-product-attribute]'),
            (index, value) => {
    ...
    setProductVariant2() {
        ...
        $.each(
            $('[data-product-option-change-2] [data-product-attribute]'),
            (index, value) => {
                const optionLabel = value.children[0].innerText;
                const optionTitle = optionLabel.split(':')[0].trim();
                const required = optionLabel.toLowerCase().includes('required');
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(
            ...
            (err, response) => {
                ...
                $.each(
                    productAttributesData.selected_attributes,
                    function (i, el) {
                        $.each(
                            $(
                                '[data-product-option-change-2] [data-product-attribute] input'
                            ),
                            function (i) {
            ...
            (err, response) => {
                ...
                if (
                    ...
                    if (this.previewModal) {
                        ...
                        if ($addToCartBtn.parents('.quickView').length === 0)
                            this.previewModal.$preModalFocusedEl =
                                $addToCartBtn;
                        this.updateCartContent(
                            this.previewModal,
                            response.data.cart_item.id,
                            () => this.previewModal.setupFocusTrap()
                        );
                        haloCalculateFreeShipping(this.context);
                    } else {
                        this.$overlay.show();
    ...
    soldProduct() {
        var numbersProduct_text =
            this.context.themeSettings.themevale_soldProduct_products;
        ...
        var numbersHours_text =
            this.context.themeSettings.themevale_soldProduct_hours;
        ...
        var soldProductText =
            this.context.themeSettings.themevale_soldProduct_text;
        var soldProductText2 =
            this.context.themeSettings.themevale_soldProduct_hours_text;

        $('.sold-product').html(
            '<svg class="icon"><use xlink:href="#icon-fire" aria-hidden="true"/></svg>' +
                '<span>' +
    ...
    initCountdown2() {
        if ($('.countDowntimer2').length) {
            var countDownDate = new Date(
                $('.countDowntimer2').attr('data-count-down2')
            ).getTime();
