...
export default function(context) {
    if (context.themeSettings.haloAddOptionForProduct) {
            ...
            $.each(aFilter, (index, element) => {
                ...
                if(aFilter3.length > 0){
                    ...
                    $.each(aFilter4, (idx, element) => {
                        ...
                        $card.find('.card-option-'+productId+'').addClass('card-option--variant');
                    });
                } else{
        ...
        $(document).on('click', btn_cardOption, e => {
            ...
            const thisTitle = $targer.find('.form-option-variant').attr('title');

            $(btn_cardOption).removeClass('is-active');
            $targer.addClass('is-active');
            $targer.parents('.card').find('.card-subTitle-color').text(thisTitle);
        });
    }
}
