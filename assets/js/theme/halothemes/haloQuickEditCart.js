    ...
    if (context.themeSettings.halo_QuickEditCart == true) {
        $(document).on('click', '[data-cart-edit-id]', (event) => {
            ...
            const modal = defaultModal(),
                options = {
                    template: 'halothemes/cart/configure-product-2',
                },
                options2 = {
                    template: 'halothemes/cart/configure-product-3',
                };
                ...
                (err2, response2) => {
                    utils.api.product.getById(
                        ...
                        (err, response) => {
                            ...
                            const $firstItem = modal.$content.find(
                                    '.product-edit-itemFirst'
                                ),
                                $form = $firstItem.find('.form'),
                                $formQty = $firstItem.find(
                            ...
                            $form
                                .find('.product-edit-option')
                                .prepend(response2.content);
        ...
        $(document).on(
            'change',
            '[data-product-edit-item-id] .form-input[cart-itemid]',
            (event) => {
                const $target = $(event.currentTarget);
        ...
        $(document).on(
            'change',
            '[data-product-edit-option-change]',
            (event) => {
        ...
        $(document).on('click', '[data-edit-cart-add-more]', (event) => {
            ...
            var count = parseInt(
                    $('.modal--editOptions [data-count]').attr('data-count')
                ),
                product = $('.modal--editOptions .product-edit-itemFirst')
                    .clone()
        ...
        $(document).on('click', '[data-edit-cart-add-to-cart]', (event) => {
            var $product = $(
                    '.modal--editOptions .product-edit-item.is-visible'
                ),
                $form = $product.find('.form'),
                itemId = $(
                    '.modal--editOptions .product-edit-itemFirst.is-visible'
                ).data('product-edit-item-id'),
                arrPro = new Array(),
                check = false;
            ...
            if (check) {
                if (arrPro.length > 0 && typeof itemId !== 'undefined') {
                    utils.api.cart.itemRemove(itemId, (err, response) => {
                        if (response.data.status === 'succeed') {
                            const originalBtnVal = $(
                                '[data-edit-cart-add-to-cart]'
                            ).text();
                            const waitMessage = $(
                                '[data-edit-cart-add-to-cart]'
                            ).data('waitMessage');

                            $('[data-edit-cart-add-to-cart]').addClass(
                                'loading'
                            );
        ...
        $(document).on(
            'click',
            '[data-cart-edit-item-quantity] button',
            (event) => {
                event.preventDefault();
                const $target = $(event.currentTarget);
        ...
        function updateProductClonedAttributes($scope, number) {
            $('label[data-product-attribute-value]', $scope).each(
                (index, attribute) => {
            ...
            $('label[data-product-attribute-value2]', $scope).each(
                (index, attribute) => {
                    var $formLabel = $(attribute),
        ...
        function setProductVariant(item, form) {
            ...
            if (productVariant) {
                ...
                $.each(productVariant, (index, value) => {
                    item.find('.product-edit-variant').append(
                        '<span>' +
                            productVariant[index].toString().split(':').pop() +
                            '</span>'
                    );
                });
            }
        ...
        function getViewModel($scope) {
            return {
                ...
                $quantity: {
                    $input: $(
                        '.product-edit-increment [name=qty\\[\\]]',
                        $scope
                    ),
                },
        ...
        function updatePriceView(viewModel, price) {
            ...
            if (price.rrp_without_tax) {
                ...
                viewModel.rrpWithoutTax.$span.html(
        ...
        function updateView($scope, data, content = null) {
            ...
            if (_.isNumber(data.stock)) {
                if (
                    data.stock <=
                        parseInt(
                            context.themeSettings.halo_stock_level_limit
                        ) &&
                    data.stock > 0
                ) {
        ...
        function refreshContent(remove) {
            ...
            utils.api.cart.getContent(options, (err, response) => {
                ...
                const quantity =
                    $('[data-cart-quantity]', $cartContent).data(
                        'cartQuantity'
                    ) || 0;
        ...
        function bindCartEvents() {
            ...
            $('.cart-item-qty-input', $cartContent)
                .on('focus', (event) => {
        ...
        function bindGiftWrappingForm() {
            ...
            function toggleViews() {
                const value = $(
                    'input:radio[name ="giftwraptype"]:checked'
                ).val();
        ...
        function bindEvents() {
            ...
            var shippingEstimator = new ShippingEstimator(
